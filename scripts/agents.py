...
from ollama_utils import retrieve_docs_with_sources, retrieve_bm25_docs, deduplicate_docs
from prompts import SUBQUESTION_PROMPT, CATEGORY_ASSIGNMENT_PROMPT, EXPAND_QUERY_PROMPT, HYDE_PROMPT, STYLE_SCORE_PROMPT, get_random_style_prompt
from ollama_chain import get_ollama_chain

# Logging-Konfiguration
def log_retrieval_process(step, data=None):
    """Logging für den Abrufprozess."""
    if data:
...
class RetrievalAgent:
    ...
    def retrieve(self, query, k=10, category=None, sparse_weight=0.3):
        ...
        dense_docs = [(doc, 1.0) for q in queries for doc in retrieve_docs_with_sources(self.vectorstore, q, k=k, category=category)]
...
class RankingAgent:
    ...
    def rerank(self, docs, query, top_k=5, style_weight=0.2):
        logging.info(f"Starte die Rangfolge der Dokumente basierend auf Relevanz und Stil.")
